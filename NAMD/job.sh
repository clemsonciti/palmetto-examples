...
cd $PBS_O_WORKDIR
mpirun -np 2 namd2 +ppn 9 +setcpuaffinity alanin > alanin.output
...
# Feel free to increase the amount of RAM (see /etc/hardware-table) if needed.
# I recommend keeping mpiprocs set to one.
# I also recommend using only one GPU (honestly I haven't tried using two GPUs yet).
